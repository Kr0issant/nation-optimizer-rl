...
def render_minister_prompt(
    ...
    return (
        ...
        "3. BUDGET PROPOSAL: Your department automatically receives its **critical minimum** funding (about 40% of demand) each round. You are now proposing **additional discretionary** funding on top of that baseline. The amount in `amount` is this extra (≥ 0). A proposal of 0 means you only need the automatic critical floor this round. The treasury has already reserved critical funding for all departments; you compete for the **remaining** treasury with other ministers' discretionary asks. Every event has a 'severity' score (1-5):\n"
        ...
        "   - `amount` on a proposal is discretionary only. Voting NO denies extra funding; the department still keeps its critical minimum for this round (it is not 'eliminated').\n"
